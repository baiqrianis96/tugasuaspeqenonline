...
class User extends MY_Controller 
{
	...
	public function user_json()
	{
		...
		$fetch			= $this->model_login->fetch_data_user($requestData['search']['value'], $requestData['order'][0]['column'], $requestData['order'][0]['dir'], $requestData['start'], $requestData['length']);
	...
	public function hapus($id_user)
	{
		...
		else
		{
			...
			{
				$this->load->model('model_login');
				$hapus = $this->model_login->hapus_user($id_user);
				if($hapus)
				{
		...
		else
		{
			if($_POST)
			{
				...
				if($this->form_validation->run() == TRUE)
				{
					...
					$insert = $this->model_login->tambah_baru($username, $password, $nama, $id_akses, $status);
	...
	{
		$this->load->model('model_login');
		$cek_user = $this->model_login->cek_username($username);
				...
				if($this->input->is_ajax_request())
				{
					...
					if($_POST)
					{
						...
						if($this->form_validation->run() == TRUE)
						{
							...
							$update = $this->model_login->update_user($id_user, $username, $password, $nama, $id_akses, $status);
							if($update)
							{
					...
					{
						$this->load->model('m_akses');
						$dt['user'] 	= $this->model_login->get_baris($id_user)->row();
						$dt['akses'] 	= $this->m_akses->get_all();
		...
		if($this->input->is_ajax_request())
		{
			if($_POST)
			{
				...
				if($this->form_validation->run() == TRUE)
				{
					...
					$update 	= $this->model_login->update_password($pass_new);
					if($update)
					{
	...
	{
		$this->load->model('model_login');
		$cek_user = $this->model_login->cek_password($pass);
